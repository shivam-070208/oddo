type ActivityStatus = "Done" | "Ready" | "Waiting" | "Draft" | "Canceled";
        ...
        <tbody className="divide-y divide-gray-50">
          {activities.map((row) => (
            <tr key={row.reference} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="py-3 pr-6 text-gray-800 font-medium">{row.product}</td>
              ...
              <td className="py-3 text-right text-gray-400 text-xs">{row.date}</td>
            </tr>
          ))}
        </tbody>
      </table>
